...
namespace CA_W12_Felicia_S
{
    public partial class Form1 : Form
    {
        ...
        private void buttonAddplay_Click(object sender, EventArgs e)
        {
            ...
            string nation = comboBoxNation.SelectedValue.ToString();
            ...
            string teamname = comboBoxTeamname.SelectedValue.ToString();
            ...
            {
                sqlQuery = $"INSERT INTO player VALUES ('{playerID}' , {teamnum}, '{name}', '{nation}', '{pos}', {height}, {weight}, '{birthdate}', '{teamname}', 1, 0);";
                try
                {
                    mysqlConnect.Open();
                    mysqlCommand = new MySqlCommand(sqlQuery, mysqlConnect);
                ...
                try
                {
                    sqlQuery = $"SELECT p.player_name as 'Name', n.nation as 'Nationality', p.playing_pos as 'Playing Position', p.team_number as 'Number', height as 'Height', p.weight as 'Weight', p.birthdate as 'Birthdate' FROM player p, nationality n WHERE p.nationality_id = n.nationality_id and p.`status` = 1 and p.team_id = '{comboBoxDeletetim.SelectedValue}';";
                    mysqlCommand = new MySqlCommand(sqlQuery, mysqlConnect);
                    mySqlAdapter = new MySqlDataAdapter(mysqlCommand);
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            count = 0;
            dataDelete2.Clear();
            sqlQuery = $"SELECT p.player_name as 'Name', n.nation as 'Nationality', p.playing_pos as 'Playing Position', p.team_number as 'Number', p.height as 'Height', p.weight as 'Weight', p.birthdate as 'Birthdate', p.`status` FROM player p, nationality n WHERE p.nationality_id = n.nationality_id and p.team_id = '{comboBoxDeletetim.SelectedValue}';";
            mysqlCommand = new MySqlCommand(sqlQuery, mysqlConnect);
            mySqlAdapter = new MySqlDataAdapter(mysqlCommand);
            ...
            {
                if (dataDelete2.Rows[i][7].ToString() == "True")
                {
                    count++;
                }
            }
            ...
            {
                sqlQuery = $"UPDATE player set `status` = 0 WHERE player_name = '{playerchoose}';";
        ...
        {
            playerchoose = dataGridViewDelete.CurrentRow.Cells[0].Value.ToString();
